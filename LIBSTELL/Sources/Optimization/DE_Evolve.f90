      SUBROUTINE DE_Evolve(obj, Dim_XC, XCmin, XCmax, VTR, NP, itermax, &
           F_XC, CR_XC, strategy, CR_strategy, refresh, iwrite,         &
           irestart, num_proc, lrestart, best_XC, bestval, nfeval)
!.......................................................................
!
...
!
      ...
      INTEGER, INTENT(in) :: NP, Dim_XC, itermax, strategy,             &
                             CR_strategy, iwrite, irestart, refresh
...
!     END DO
      ...
      IF (myid .eq. master)                                             &
          WRITE(6,*)'initial best=',bestval,' at loc ',ibest
...
!===========================================================================
        ...
        CASE (1)
           ...
           ui_XC=ui_XC+F_XC*(pop_XC(a1,:)-pop_XC(a2,:))

        CASE DEFAULT
           WHERE(lcross)                                                &
           ui_XC=pop_XC(a3,:)+F_XC*(pop_XC(a1,:)-pop_XC(a2,:))

        CASE (3)
           WHERE(lcross)                                                &! note: in these locations ui_XC = best CASE
           ui_XC=pop_XC+F_XC*(ui_XC-pop_XC+pop_XC(a1,:)-pop_XC(a2,:))

        CASE (4)
           WHERE(lcross)                                                &! note: in these locations ui_XC = best CASE
           ui_XC=ui_XC+F_XC*(pop_XC(a1,:)-pop_XC(a2,:)+pop_XC(a3,:)     &
                           -pop_XC(a4,:))

        CASE (5)
           WHERE(lcross)                                                &
           ui_XC=pop_XC(a5,:)+F_XC*(pop_XC(a1,:)-pop_XC(a2,:)+          &
                                    pop_XC(a3,:)-pop_XC(a4,:))
...
!------Write output summary IF needed------------------------------
        ...
        IF( refresh > 0 ) THEN
           IF (MOD(iter,refresh) == 0) THEN
              ...
              IF (myid .eq. master) THEN
!          WRITE(unit=iwrite,FMT=203) iter
                 WRITE(6, FMT=203) iter
                 DO i=1,Dim_XC
!                WRITE(unit=iwrite, FMT=202) i, best_XC(i)
...
!          WRITE(unit=iwrite, FMT=201) bestval
        ...
        IF ( bestval <= VTR ) THEN
!          WRITE(iwrite, *) ' The best fitness is smaller than VTR'
           IF (myid .eq. master)                                        &
              WRITE(6, *) 'The best fitness is smaller than VTR'
           EXIT
        END IF
