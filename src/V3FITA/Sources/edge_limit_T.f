...
!-------------------------------------------------------------------------------
      INTERFACE ASSIGNMENT (=)
         MODULE PROCEDURE edge_limit_desc_assign
      END INTERFACE
!-------------------------------------------------------------------------------
...
!    limiter_iso component has pointer inside.
!-------------------------------------------------------------------------------
      SUBROUTINE edge_limit_desc_assign(left,right)

      IMPLICIT NONE
      
!  Declare Arguments 
      TYPE (edge_limit_desc), INTENT (inout) :: left
      TYPE (edge_limit_desc), INTENT (in) :: right
      
!  Declare local variables
      CHARACTER(len=*), PARAMETER :: sub_name =                                &
     &  'edge_limit_desc_assign: '
...
!  Destroy left
      CALL edge_limit_destroy(left)
      
      left % name = right % name
      left % edge_limit_type = right % edge_limit_type
      left % units = right % units
      left % sigma_default = right % sigma_default
      left % l_on_edge = right % l_on_edge
      left % lim_iso = right % lim_iso

      END SUBROUTINE edge_limit_desc_assign
...
!  Select Case of Verbosity Level
      ...
      CASE(1:)    ! Default, more verbose
         ...
         WRITE(iou,fmt1(7)) id
         CALL limiter_iso_write(this % lim_iso,'eldw',iou,2)
...
!
!  JDH 2011-03-16
!    Added explicit assignment for edge_limit_desc, because it contains
!    a limiter_iso, which contains a pointer (vgrid). Also, added call
!    to limiter_iso_write in edge_limit_desc_write
!
