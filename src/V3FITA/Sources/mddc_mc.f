...
!      ELSE
  !!       sumtot = -sumtot*delsuv      ! Commented out 2012-04-17 JDH
!      END IF

!  JDH 2012-04-17. Add factor of the Jacobian sign.
!    Previously, magnetic diagnostic plasma signals had the correct sign for
!    left-handed (negative Jacobian) s-u-v coordinate systems, and a sign 
!    error fo right-handed (negative Jacobian) s-u-v coordinate systems.
!
!    I commented out the 'sumtot = -sumtot*delsuv' statement above
!    (that was in the midst of a commented out 'IF (lsurf)' block)
!    I assume that the minus sign in 'sumtot = -sumtot*delsuv' was to correct
!    for the Jacobian sign for lhs s-u-v coordinates. (There is a factor of 
!    sqrt(g) in the definition of the currusuv and currvsuv factors. )
!
!    Here I explicitly multiply by the sign of the Jacobian (isigng)
!    This code should work for both lhs and rhs s-u-v coordinates.
!  JDH 2012-04-17. END Add factor of the Jacobian sign.

      sumtot = state % aux1 % isigng * sumtot * delsuv
...
!
!  JDH 2012-04-17
!    Added factor of the sign of the Jacobian to the plasm signal computation
!
