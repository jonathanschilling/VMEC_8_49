...
!  Declare local variables
      ...
      INTEGER                  :: i1, i2, ilb1, ilb2, iub1, iub2
...
!-------------------------------------------------------------------------------
     ...
     &         'iam range max18')
         ...
         CASE ('rbc')
            i1 = rparam(i) % index
            i2 = rparam(i) % index2
            ilb1 = LBOUND(state % varp % rbc,1)
            ilb2 = LBOUND(state % varp % rbc,2)
            iub1 = UBOUND(state % varp % rbc,1)
            iub2 = UBOUND(state % varp % rbc,2)
            CALL assert_eq(ilb1,MIN(ilb1,i1),sub_name // 'rbc r min1')
            CALL assert_eq(ilb2,MIN(ilb2,i2),sub_name // 'rbc r min2')
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'rbc r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'rbc r max2')
            rparam(i) % value = state % varp % rbc(i1,i2)

         CASE ('rbs')
            i1 = rparam(i) % index
            i2 = rparam(i) % index2
            ilb1 = LBOUND(state % varp % rbs,1)
            ilb2 = LBOUND(state % varp % rbs,2)
            iub1 = UBOUND(state % varp % rbs,1)
            iub2 = UBOUND(state % varp % rbs,2)
            CALL assert_eq(ilb1,MIN(ilb1,i1),sub_name // 'rbs r min1')
            CALL assert_eq(ilb2,MIN(ilb2,i2),sub_name // 'rbs r min2')
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'rbs r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'rbs r max2')
            rparam(i) % value = state % varp % rbs(i1,i2)

         CASE ('zbc')
            i1 = rparam(i) % index
            i2 = rparam(i) % index2
            ilb1 = LBOUND(state % varp % zbc,1)
            ilb2 = LBOUND(state % varp % zbc,2)
            iub1 = UBOUND(state % varp % zbc,1)
            iub2 = UBOUND(state % varp % zbc,2)
            CALL assert_eq(ilb1,MIN(ilb1,i1),sub_name // 'zbc r min1')
            CALL assert_eq(ilb2,MIN(ilb2,i2),sub_name // 'zbc r min2')
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'zbc r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'zbc r max2')
            rparam(i) % value = state % varp % zbc(i1,i2)

         CASE ('zbs')
            i1 = rparam(i) % index
            i2 = rparam(i) % index2
            ilb1 = LBOUND(state % varp % zbs,1)
            ilb2 = LBOUND(state % varp % zbs,2)
            iub1 = UBOUND(state % varp % zbs,1)
            iub2 = UBOUND(state % varp % zbs,2)
            CALL assert_eq(ilb1,MIN(ilb1,i1),sub_name // 'zbs r min1')
            CALL assert_eq(ilb2,MIN(ilb2,i2),sub_name // 'zbs r min2')
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'zbs r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'zbs r max2')
            rparam(i) % value = state % varp % zbs(i1,i2)
...
!            rparam(i) % value = a_model % density % tau
         CASE ('pp_ne_b')
            CALL pprofile_get(a_model % pp_ne,'b',rparam(i) % index,           &
     &         rparam(i) % value)
            
         CASE ('pp_ne_as')
            CALL pprofile_get(a_model % pp_ne,'as',rparam(i) % index,          &
     &         rparam(i) % value)
            
         CASE ('pp_ne_af')
            CALL pprofile_get(a_model % pp_ne,'af',rparam(i) % index,          &
     &         rparam(i) % value)
            
         CASE ('pp_sxrem_b')
            CALL pprofile_get(a_model % pp_sxrem,'b',rparam(i) % index,        &
     &         rparam(i) % value)
            
         CASE ('pp_sxrem_as')
            CALL pprofile_get(a_model % pp_sxrem,'as',                         &
     &         rparam(i) % index,rparam(i) % value)
            
         CASE ('pp_sxrem_af')
            CALL pprofile_get(a_model % pp_sxrem,'af',                         &
     &         rparam(i) % index,rparam(i) % value)
            
         CASE ('pp_te_b')
            CALL pprofile_get(a_model % pp_te,'b',rparam(i) % index,           &
     &         rparam(i) % value)
            
         CASE ('pp_te_as')
            CALL pprofile_get(a_model % pp_te,'as',                            &
     &         rparam(i) % index,rparam(i) % value)
            
         CASE ('pp_te_af')
            CALL pprofile_get(a_model % pp_te,'af',                            &
     &         rparam(i) % index,rparam(i) % value)
...
!-------------------------------------------------------------------------------
     ...
     &         'iam range max18')
         ...
         CASE ('rbc')
            ...
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'rbc r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'rbc r max2')
            state % varp % rbc(i1,i2) = rparam_a(i) % value
         ...
         CASE ('rbs')
            ...
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'rbs r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'rbs r max2')
            state % varp % rbs(i1,i2) = rparam_a(i) % value
         ...
         CASE ('zbc')
            ...
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'zbc r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'zbc r max2')
            state % varp % zbc(i1,i2) = rparam_a(i) % value
         ...
         CASE ('zbs')
            ...
            CALL assert_eq(iub1,MAX(iub1,i1),sub_name // 'zbs r max1')
            CALL assert_eq(iub2,MAX(iub2,i2),sub_name // 'zbs r max2')
            state % varp % zbs(i1,i2) = rparam_a(i) % value
...
!
         CASE ('pp_ne_b')
            CALL pprofile_put(a_model % pp_ne,'b',rparam_a(i) % index,         &
     &         rparam_a(i) % value)
            
         CASE ('pp_ne_as')
            CALL pprofile_put(a_model % pp_ne,'as',rparam_a(i) % index,        &
     &         rparam_a(i) % value)
            
         CASE ('pp_ne_af')
            CALL pprofile_put(a_model % pp_ne,'af',rparam_a(i) % index,        &
     &         rparam_a(i) % value)
            
         CASE ('pp_sxrem_b')
            CALL pprofile_put(a_model % pp_sxrem,'b',                          &
     &         rparam_a(i) % index,rparam_a(i) % value)
            
         CASE ('pp_sxrem_as')
            CALL pprofile_put(a_model % pp_sxrem,'as',                         &
     &         rparam_a(i) % index,rparam_a(i) % value)
            
         CASE ('pp_sxrem_af')
            CALL pprofile_put(a_model % pp_sxrem,'af',                         &
     &         rparam_a(i) % index,rparam_a(i) % value)
            
         CASE ('pp_te_b')
            CALL pprofile_put(a_model % pp_te,'b',                             &
     &         rparam_a(i) % index,rparam_a(i) % value)
            
         CASE ('pp_te_as')
            CALL pprofile_put(a_model % pp_te,'as',                            &
     &         rparam_a(i) % index,rparam_a(i) % value)
            
         CASE ('pp_te_af')
            CALL pprofile_put(a_model % pp_te,'af',                            &
     &         rparam_a(i) % index,rparam_a(i) % value)
...
!
!  JDH 2011-01-03
!      Added p_types for boundary coefficients, rbc, zbc, rbs, zbs
!
!  JDH 2011-10-19
!     Added p_type for parameterized profile-ne ('pp_ne_b')
!
!  JDH 2011-10-19
!     Added 'pp_ne_as' and 'pp_ne_af' - ne spline coefficients
!
!  JDH 2011-10-19
!     Added 'pp_sxrem_b', _as, and _af - sxr emissivity spline coefficients
!
!  JDH 2011-10-23
!     Added 'pp_te_b', _as, and _af - sxr emissivity spline coefficients
