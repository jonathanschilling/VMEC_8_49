...
!       rbc         - VMEC2000 R-Cosine boundary specification (LFREEB = F)
!       rbs         - VMEC2000 R-Sine boundary specification (LFREEB = F)
!       zbc         - VMEC2000 Z-Cosine boundary specification (LFREEB = F)
!       zbs         - VMEC2000 Z-Sine boundary specification (LFREEB = F)
...
!    (JDH 2011-10-19 - switch from density_max and tau to pp_ne_b - 
!      parameterized profile coefficients for ne profile)
!       pp_ne_b        - model electron density parameterized profile coefficients
!       pp_ne_as       - model electron density spline s values (independent variable)
!       pp_ne_af       - model electron density spline f values (dependent variable)
!       pp_sxrem_b     - model sxr emissivity parameterized profile coefficients
!       pp_sxrem_as    - model sxr emissivity spline s values (independent variable)
!       pp_sxrem_af    - model sxr emissivity spline f values (dependent variable)
!       pp_te_b        - model electron temperature parameterized profile coefficients
!       pp_te_as       - model electron temperature spline s values (independent variable)
!       pp_te_af       - model electron temperature spline f values (dependent variable)
!    
...
!       index2          integer, second index to an array, if needed
...
!
!-------------------------------------------------------------------------------
      SUBROUTINE recon_param_construct(this,p_type,value,index,                &
     &   index2,vrnc,fdf)
...
!  Actual Assignments    
      ...
      this % index(n) = index                     ! Add (n) 2011-11-03 JDH
      this % value(n) = value                     ! Add (n) 2011-11-03 JDH
...
!  Subroutine to put the finite difference factor
...
!  Select Case of Verbosity Level
      ...
      CASE( :0)  ! VERY Terse
         WRITE(iou,1200)
         DO i = 1,size(this)
            WRITE(iou,1300) i, this(i)%p_type, this(i)%index,                  &
     &         this(i)%index2, this(i)%value, this(i)%vrnc, this(i)%fdf
         END DO
...
1100  FORMAT(" start recon_param array write, called with id = ",a)
1200  FORMAT(" i     %p_type  %index %index2   %value",t50,"%vrnc",            &
     &   t64,"fdf")
1300  FORMAT(i4,2x,a10,2x,i4,2x,i4,2x,es12.5,2x,es12.5,2x,es12.5)
1400  FORMAT(" end recon_param array write, called with id = ",a)
...
!  Select Case of Verbosity Level
     ...
     &      old(i) % index2,old(i) % value, new(i) % value,                    &
...
1000  FORMAT(/'Reconstruction Parameters, old, new, difference '/              &
     &   'irp',t7,'type',t16,'index index2',t32,                               &
     &   'old',t46,'new',t58,'diff')
1100  FORMAT(i3,2x,a10,2x,i3,2x,i3,2x,es11.4,2x,es11.4,2x,es11.4)
...
!
!  JDH 2011-01-03
!      Added index2 to recon_param, to accomodate new types:
!        rbc, rbs, zbc, zbs
!
!  JDH 2011-01-11
...
!
!  JDH 2011-10-19
!      Added type 'pp_ne_b'. Eliminated Select Case in recon_param_destroy.
!      Eliminate types 'density_max' and 'density_tau'
!
!  JDH 2011-10-21
!      Added type 'pp_ne_as' and 'pp_ne_as'. (Splines)
!
!  JDH 2011-10-23
!      Added type 'pp_sxrem_b', _as and _af. (Splines for sxr emissivity)
!
!  JDH 2011-10-23
!      Added type 'pp_te_b', _as and _af. (Splines for electron temperature)
!
!  JDH 2011-11-03
!      Fix bug in recon_cnstrnts_append.
