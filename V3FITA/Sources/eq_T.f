...
! SECTION VIII.   GET SUBROUTINES
! SECTION IX  .   OTHER SUBROUTINES
!*******************************************************************************
      MODULE eq_T
...
!-------------------------------------------------------------------------------
!  Use vmec_input ndatafmax for auxiliary arrays (J Geiger, 20120203)
!-------------------------------------------------------------------------------
      USE vparams, ONLY: ndatafmax
...
!  Privacy
!-------------------------------------------------------------------------------
!  the aux_construct routines should only be called from the aux_define routines
      PRIVATE eq_aux_1_construct, eq_aux_2_construct
...
!                                                           eqsolve.f
!       rbc_bounds   integer(4) - Bounds of the rbc, rbs, zbc, and zbs arrays
...
! 			           VMEC:vmec_input:vmec_input.f
! J Geiger hard-coding creates difficulties!
...
!  Variables for code='VMEC2000'
         INTEGER ::  ns_index
         INTEGER ::  rbc_bounds(4)                            ! Added 2011-01-03
         REAL(rprec), DIMENSION(0:20) :: ac
         REAL(rprec), DIMENSION(ndatafmax)  :: ac_aux_s       ! Added 2010-12-04, J Geiger 2012-02-03
         REAL(rprec), DIMENSION(ndatafmax)  :: ac_aux_f       ! Added 2010-12-04, J Geiger 2012-02-03
         REAL(rprec), DIMENSION(0:20) :: ai
         REAL(rprec), DIMENSION(ndatafmax)  :: ai_aux_s       ! Added 2010-12-04, J Geiger 2012-02-03
         REAL(rprec), DIMENSION(ndatafmax)  :: ai_aux_f       ! Added 2010-12-04, J Geiger 2012-02-03
         REAL(rprec), DIMENSION(0:20) :: am
         REAL(rprec), DIMENSION(ndatafmax)  :: am_aux_s       ! Added 2010-12-04, J Geiger 2012-02-03
         REAL(rprec), DIMENSION(ndatafmax)  :: am_aux_f       ! Added 2010-12-04, J Geiger 2012-02-03
...
!  ns
!  rzl            4d array of r,z coefficients
!                    generated in subroutine LoadRZL of read_wout_file
!  lthreed        logical, defined in LoadRZL, three dimensional (or axisymmetric)
!  ntmax          integer, defined in LoadRZL, for length of rzl 4th dim.
!  isigng         integer, sign of the jacobian of the (s,u,v) coordinate system
!  
!-------------------------------------------------------------------------------
      TYPE eq_aux_1
!  Variables for code='VMEC'
...
!  Variables defined in LoadRZL
!  rzl array - for arguments to subroutines in module cyl_flux
         REAL(rprec), DIMENSION(:,:,:,:), POINTER :: rzl => null()
         INTEGER                         :: ntmax
         LOGICAL                         :: lthreed
...
!
!-------------------------------------------------------------------------------
      SUBROUTINE eq_param_var_construct(this, ns_index, rbc_bnds,              &
...
!  Declare Arguments 
      ...
      INTEGER, DIMENSION(4), INTENT(in)       :: rbc_bnds
      ...
      REAL(rprec),                                                             &
     &   DIMENSION(rbc_bnds(1):rbc_bnds(2),rbc_bnds(3):rbc_bnds(4)),           &
     &   INTENT(in) :: rbc, zbs, zbc, rbs
...
!  Declare local variables
      INTEGER :: nd1, nd2, ier1, ilb1, iub1, ilb2, iub2
      CHARACTER(len=*), PARAMETER :: sub_name =                                &
...
!  For ac, ai, and am do vector assignment. Should get correct lower limit.
     ...
     &   sub_name // 'am_aux_f array sizes unequal','Warn')
      ...
      ilb1 = this % rbc_bounds(1)
      iub1 = this % rbc_bounds(2)
      ilb2 = this % rbc_bounds(3)
      iub2 = this % rbc_bounds(4)
      
      ALLOCATE(this % rbc(ilb1:iub1,ilb2:iub2),STAT=ier1)
      CALL assert_eq(0,ier1,sub_name // 'alloc rbc')
      this % rbc = rbc

      ALLOCATE(this % zbs(ilb1:iub1,ilb2:iub2),STAT=ier1)
      CALL assert_eq(0,ier1,sub_name // 'alloc zbs')
      this % zbs = zbs

      ALLOCATE(this % zbc(ilb1:iub1,ilb2:iub2),STAT=ier1)
      CALL assert_eq(0,ier1,sub_name // 'alloc zbc')
      this % zbc = zbc

      ALLOCATE(this % rbs(ilb1:iub1,ilb2:iub2),STAT=ier1)
...
!
!  This subroutine should be PRIVATE, and only called from eq_aux_1_define
!  The testing of this % l_def_aux1 is done in eq_aux_1_define.
!
!  2011-09-07. Add the rzl array, and the call to LoadRZL.
!
...
!  duplicate the coding in VMEC that converts from the state variables
     ...
     &   rzl_local, ntmax, lthreed, isigng
...
!  Copy arrays in to this
      ...
      dims(1:4) = shape(rzl_local)
      ALLOCATE(this % rzl(dims(1),dims(2),dims(3),dims(4)))
      this % rzl = rzl_local
...
!  Declare local variables
     ...
     & 'left-right pointers are the same?. FIX IT'
      ...
      INTEGER :: n, nm1, ilb1, iub1, ilb2, iub2
...
!  Copy them over
      ...
      left % rbc_bounds = right % rbc_bounds
      left % ac = right % ac
...
!  Allocate space for arrays (Were deallocated in _destroy)
      IF (ASSOCIATED(right % rbc)) THEN
         ilb1 = right % rbc_bounds(1)
         iub1 = right % rbc_bounds(2)
         ilb2 = right % rbc_bounds(3)
         iub2 = right % rbc_bounds(4)
      
         ALLOCATE(left % rbc(ilb1:iub1,ilb2:iub2),STAT=ier1)
         CALL assert_eq(0,ier1,sub_name // 'alloc rbc')
         left % rbc = right % rbc
      
         ALLOCATE(left % zbs(ilb1:iub1,ilb2:iub2),STAT=ier1)
         CALL assert_eq(0,ier1,sub_name // 'alloc zbs')
         left % zbs = right % zbs
      
         ALLOCATE(left % zbc(ilb1:iub1,ilb2:iub2),STAT=ier1)
         CALL assert_eq(0,ier1,sub_name // 'alloc zbc')
         left % zbc = right % zbc
      
         ALLOCATE(left % rbs(ilb1:iub1,ilb2:iub2),STAT=ier1)
         CALL assert_eq(0,ier1,sub_name // 'alloc rbs')
         left % rbs = right % rbs
      ELSE
         left % rbc => null()
...
!  Copy them over
      ...
      left % ns = right % ns
      left % ntmax = right % ntmax
      left % lthreed = right % lthreed
...
!  Check that rzl array is associated
      IF (ASSOCIATED(right % rzl)) THEN
         dims(1:4) = SHAPE(right % rzl)
         ALLOCATE(left % rzl(dims(1),dims(2),dims(3),dims(4)))
         left % rzl = right % rzl
      ELSE
         left % rzl => null()
      ENDIF
...
!  Start of executable coded
      IF (this % l_def_aux1) THEN
         WRITE(*,*) 'l_def_aux1 T at call to eq_aux_1_define. FIX'
      ENDIF
...
!  Start of executable code
      ...
      IF (this % l_def_aux2) THEN
         WRITE(*,*) 'l_def_aux2 T at call to eq_aux_2_define. FIX'
      ENDIF
...
!-------------------------------------------------------------------------------
!  UnDefine both the aux_1 and aux_2
!
!-------------------------------------------------------------------------------
      SUBROUTINE eq_aux_12_undefine(this)

!  2011-09-07
!  Set the logical variables l_def_aux1 and l_def_aux2 to .false.
!  to indicate that the aux1 and aux2 structures need to be redefined.
      IMPLICIT NONE
...
!  Start of executable code
      ...
      END SUBROUTINE eq_aux_12_undefine

!*******************************************************************************
! SECTION VIII.   GET SUBROUTINES
!*******************************************************************************
!-------------------------------------------------------------------------------
!  Get R and Z on the plasma boundary (fixed toroidal angle)
!
!-------------------------------------------------------------------------------
      SUBROUTINE eq_get_seq1_rz(this,phi,r_at_seq1,z_at_seq1)

!  Subroutine to return the R and Z positions of points on the
!  plasma surface (s = 1 surface), on a constant_phi plane.
!  Based on an earlier subroutine, edge_limit_mc_aux_rz_one
!  The r_at_seq1 and z_at_seq1 arrays must be the same length.

!    Arguments
!  this          An eq_state (inout)
!  phi           Toroidal angle, in radians for s=1 surface. (in)
!  r_at_seq1     Array of R values on the s=1, phi constant curve (inout)
!  z_at_seq1     Array of Z values on the s=1, phi constant curve (inout)

      IMPLICIT NONE
      
!  Declare Arguments 
      TYPE (eq_state), TARGET, INTENT (inout)          :: this
...
!-- Start of executable code                                         --
!----------------------------------------------------------------------
! See how big the r and z arrays are
      nu = SIZE(r_at_seq1)
      nu2 = SIZE(z_at_seq1)
      CALL assert_eq(nu,nu2,sub_name // 'size r, z mismatch')
...
!  Consistency checks need to be done
      lasym = this % fixp % lasym
!  Check to see that aux1 is in correct state.
      ns = SIZE(aux1 % rmnc,2)
      mnmax = size(aux1 % xm,1)   ! should be same for both xm and xn
      delu = twopi / nu
!----------------------------------------------------------------------
!-- allocate work arrays                                             --
!----------------------------------------------------------------------
!  u VMEC flux grid
      ALLOCATE (ugrid(nu),stat=i)
      CALL assert_eq(0,i,sub_name // 'alloc ugrid')

!  Sin and cosine arrays for u and v coordinates
!    Note, since only using one v (phi) coordinate, cosv is 1d
      ALLOCATE (cosu(mnmax, nu), sinu(mnmax, nu),stat=i)
      CALL assert_eq(0,i,sub_name // 'alloc cosu')

      ALLOCATE (cosv(mnmax), sinv(mnmax),stat=i)
      CALL assert_eq(0,i,sub_name // 'alloc cosv')

      ALLOCATE (cosz(mnmax), sinz(mnmax),stat=i)
      CALL assert_eq(0,i,sub_name // 'alloc cosz')

!----------------------------------------------------------------------
!-- set up (s=1,u,v=phi) VMEC flux grids                             --
!----------------------------------------------------------------------
!  index ns corresponds to s = 1
      DO j = 1, nu
         ugrid(j) = (j - 1) * delu
         cosu(:,j) = COS(aux1 % xm * ugrid(j))
         sinu(:,j) = SIN(aux1 % xm * ugrid(j))
      END DO
      cosv(:) = COS(aux1 % xn * phi)
      sinv(:) = SIN(aux1 % xn * phi)
!
!       DO FOURIER MODE SUM 
!
      DO j = 1, nu
!          zetamn = xm*ugrid(j) - xn*vgrid
!          cosz = COS(zetamn)
!          sinz = SIN(zetamn)
         cosz = cosu(:,j) * cosv(:) + sinu(:,j) * sinv(:)
         sinz = sinu(:,j) * cosv(:) - cosu(:,j) * sinv(:)
         r_at_seq1(j) = SUM(aux1 % rmnc(:,ns) * cosz)
         z_at_seq1(j) = SUM(aux1 % zmns(:,ns) * sinz)

!----------------------------------------------------------------------
!-- stellarator asymmetric terms                                     --
!----------------------------------------------------------------------
         IF (lasym) THEN
            r_at_seq1(j) = r_at_seq1(j) + SUM(aux1 % rmns(:,ns)*sinz)
            z_at_seq1(j) = z_at_seq1(j) + SUM(aux1 % zmnc(:,ns)*cosz)
         ENDIF
      END DO

!  Deallocate space and nullify pointers
      DEALLOCATE (cosz, sinz, cosu, cosv, sinu, sinv, ugrid)
      aux1 => null()

      RETURN
      END SUBROUTINE eq_get_seq1_rz

!-------------------------------------------------------------------------------
!  Get pressure
!-------------------------------------------------------------------------------
      FUNCTION eq_get_pressure_s(this,s_arg)
!  FUNCTION to return the pressure, given a model and a value of
!  the radial coordinate s.
      IMPLICIT NONE
      
!  Declare Arguments
      REAL(rprec)                              :: eq_get_pressure_s
      TYPE (eq_state), INTENT (inout)          :: this
...
!  Interface to a function located in file eq_interface.f,
!  (But not actually IN the eq_interface module.)
      INTERFACE 
...
!  Two problems here (JDH 2011-07-27)
!  1) VMEC does not really know the pressure, it knows the mass.
!        Resolve by ASSUMING the mass is the pressure.
!        To Do - add coding to check gamma, emit warning if mass != pressure
!  2) Will pass this on to the VMEC subroutine, so it is not getting the
!     pressure of this, but rather the pressure of the VMI state.
!        To Do - add coding to a) put info into VMI, compute p, restore VMI
!        OR - duplicate coding for pressure
!        OR - use arguments to pressure function (BEST WAY - multi-arg functions)

      eq_get_pressure_s = eq_interface_get_pressure_s(s_arg)
      
      RETURN
         
      END FUNCTION eq_get_pressure_s
!-------------------------------------------------------------------------------
!  Get pressure for cartesian position
!-------------------------------------------------------------------------------
      FUNCTION eq_get_pressure_x(this,x_arg)
!  FUNCTION to return the pressure, given a model and values of the 
!  cartesian coordinates x_arg.
      IMPLICIT NONE
      
!  Declare Arguments
      REAL(rprec)                              :: eq_get_pressure_x
      TYPE (eq_state), INTENT (inout)          :: this
      REAL (rprec), DIMENSION(3), INTENT (in)  :: x_arg
...
!  Find the VMEC s coordinate
      s = 1.  !  So that it compiles

!  Using the s coordinate, get the pressure
      eq_get_pressure_x = eq_get_pressure_s(this,s)
      
      RETURN
               
      END FUNCTION eq_get_pressure_x
!-------------------------------------------------------------------------------
!  Get vmec flux coordinates
!-------------------------------------------------------------------------------
        SUBROUTINE eq_get_flux(this,x_cylin,c_flux)

!  Subroutine to return the VMEC s,u,v coordinates (in c_flux)
!  for an equilibrium state and cylindrical coordinates R,phi,Z
!  (in x_cylin)

!  Works by calling the cyl2flx subroutine in LIBSTELL/cyl_flux

!    Arguments
!  this          An eq_state (inout)
!  x_cylin       Array of cylindrical coordinates R, Phi, Z (in)
!  c_flux        Array of vmec coordinates s, u, v (out)

!-------------------------------------------------------------------------------
!  Note on Angles:
!
!  1) Define phi to be the geometrical toroidal angle, phi = atan(y/x)
!  with 0 <= phi < 2 pi
!
!  2) Because of the field periodicity (with # field periods = N), it is
!  convenient to translate the toroidal angle into the first field period.
!  phi_1 = phi Mod (2 pi / N) = phi - m 2 pi / N, where
!  (m 2 pi / N) <= phi < ((m + 1) 2 pi / N)
!
!  3) It is also convenient to have an angle v that increases from 0 to 2 pi
!  within the first field period: v = N phi_1 = N phi - 2 pi m
!
!  4) We see that v = N phi Mod (2 pi). Since we really only use Sines 
!  and Cosines of v, we can say (imprecisely) that v = N phi
!-------------------------------------------------------------------------------
...
!  Declare Arguments 
      TYPE (eq_state), TARGET, INTENT (inout)          :: this
      REAL(rprec), INTENT (in)                         :: x_cylin(3)
      REAL(rprec), INTENT(inout)                       :: c_flux(3)
...
!  Declare local variables
      REAL(rprec)                         :: r_cyl(3)
      INTEGER :: ns_local, ntor_local, mpol_local, ntmax_local,
     &  info_local, nfe_local
      LOGICAL :: lasym_local, lthreed_local
      REAL(rprec)    :: fmin_local
      
      REAL(rprec), PARAMETER :: fmin_accept = 1.E-8_rprec
...
!  Set variables for call the LIBSTELL subroutine cyl2flx
      ns_local = aux1 % ns
      ntor_local = this % fixp % ntor
      mpol_local = this % fixp % mpol
      ntmax_local = aux1 % ntmax
      lthreed_local = aux1 % lthreed
      lasym_local = this % fixp % lasym
      info_local = 0
      nfe_local = 0
      fmin_local = zero
      
      r_cyl(1) = x_cylin(1)
      r_cyl(2) = this % fixp % nfp * x_cylin(2)   ! Appropriate argument for cyl2flx
      r_cyl(3) = x_cylin(3)

      CALL cyl2flx(aux1 % rzl, r_cyl, c_flux, ns_local, ntor_local,            &
     &   mpol_local,ntmax_local,lthreed_local,lasym_local,info_local,          &
     &   nfe_local,fmin_local)
        
        
!  info_local = -1, did not converge
!  info_local = -3, s > 1

!  Don't warn if s > 1
!  Warn if distance is WORSE than 0.1 mm

      IF (c_flux(1) .ge. one) RETURN
      IF (fmin_local .gt. fmin_accept) THEN
         WRITE(*,*) "  Warn: ", sub_name, info_local, fmin_local,              &
     &      c_flux(1)
      ENDIF

      RETURN
      END SUBROUTINE eq_get_flux
...
!
!  JDH 2011-01-03
!    Updated eq_param_var to accomodate using rbc, zbs, zbc, and rbs as
!    reconstruction parameters. Need to get appropriate arrya bounds.
!  
!  JDH 2011-07-21
!    Added eq_get_seq1_rz
!  
!  JDH 2011-07-28
!    Added eq_get_pressure_s
!  
!  JDH 2011-09-07
!    Add rzl, lthreed, ntmax to aux_1. Subroutine to undefine the aux structures.
!    Add subroutine eq_get_pressure_x (not yet functional)
!    Add subroutine eq_get_flux - interface to cyl2flx in LIBSTELL module
!    cyl_flux.
!
!  JDH 2011-10-20
!    Compiler on blast found error, can't do Logical_var .eq. .true.
!    Fixed
!
!  JDH 2011-10-25
!    Fixed up coding in eq_param_var_assign to take care of case when
!    the pointers are not yet associated.
!
!  JDH 2011-11-09
!    In eq_aux_1_assign, change dimension of dims from 3 to 4, to accomodate
!    rzl.
!
!  J Geiger 2012-02-03 
!    (Comment written by JDH) Added 
!        USE vparams, ONLY: ndatafmax
!    and declarations, so that auxiliary arrays for splines have correct length 

!  JDH 2012-04-16
!    Add sign of jacobian variable to eq_aux_1 derived type. Added appropriate
!    coding to eq_aux_1_construct to get the variable from the wout file.
